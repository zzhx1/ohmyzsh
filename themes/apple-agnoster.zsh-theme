...
# Conda env (or Apple logo if no conda env active)
prompt_apple() {
  local conda_env="${CONDA_DEFAULT_ENV:-}"
  if [[ -n "$conda_env" ]]; then
    prompt_segment black default "$conda_env"
  else
...
# Virtualenv: current working virtualenv (conda display removed)
prompt_virtualenv() {
...
# Prevent conda from prepending (env) to the prompt
# We display the conda env in prompt_apple instead
_agnoster_reset_prompt() {
  PROMPT=$'%{%f%b%k%}$(build_prompt)\n%F{red}❯%F{yellow}❯%F{green}❯%f '
}
precmd_functions+=(_agnoster_reset_prompt)
